...
const (
	...
	ByeByeStr                   = "delete from users where uid = ?"
)

func DisRegister(uid string) error {
	if _, err := g.Xdb.Exec(ByeByeStr, uid); err != nil {
		return err
	}
	return nil
}
...
func CheckPassword(password string, uid string) error {
	...
	password = untils.ComplexPassword(password)
